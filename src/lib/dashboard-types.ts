...
export type FundTechnical = {
  ...
  relStrengthVsNifty20D: number | null;
  relStrengthVsNifty50D: number | null;
};
...
export type ScoredFund = {
  ...
  proxyKey: string;
  proxyLabel: string;
  proxyMovePct: number | null;
  strategicScore: number;
  opportunityScore: number;
  finalScore: number;
  trend: TrendState;
  classification: FundClassification;
  actionTag: FundActionTag;
  reason: string;
  expectedNavImpactNote: string;
  technical: FundTechnical;
};

export type Regime = {
  badge: "RISK_ON" | "RISK_OFF";
  label: string;
  strategyNote: string;
...
export type TacticalAllocationItem = {
  fundId: string;
  fundName: string;
  amount: number;
  scoreWeight: number;
...
export type DashboardSnapshot = {
  ...
  regime: Regime;
  headlineIndices: IndexQuote[];
  fallingIndices: IndexQuote[];
  strongestIndices: IndexQuote[];
  topFunds: ScoredFund[];
  avoidFunds: ScoredFund[];
  allFunds: ScoredFund[];
  sectorHeatmap: IndexQuote[];
  indexTable: IndexQuote[];
  tacticalAllocation: TacticalAllocationItem[];
  cutoffProgressPct: number;
  cutoffLabel: string;
  disclaimers: string[];
};
